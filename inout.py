...
def heatmaps(
    matrix: NDArray[np.float64], title: str, colored: bool, bounded: bool = False
) -> Figure:
    ...
    if bounded:
        sns.heatmap(
            matrix, vmin=-1, vmax=1, center=center, cmap=cmap, square=True, ax=ax
        )
    else:
...
def get_schaefer100_fc(subject_id: str) -> cp.ndarray:
    ...
    fc_path = (
        path
        + f"func/sub-HC{subject_id}_ses-01_space-fsnative_atlas-schaefer100_desc-fc.txt"
    )
...
def get_schaefer100_sc(subject_id: int) -> cp.ndarray:
    """Gets the structural connectivity matrix."""
    subject_id_zero_padded: str = (
        f"00{subject_id}" if subject_id < 10 else f"0{subject_id}"
    )
    path = f"data/micapipe/sub-HC{subject_id_zero_padded}/ses-01/"
    sc_path = (
        path
        + f"dwi/sub-HC{subject_id}_ses-01_space-dwinative_atlas-schaefer100_desc-sc.txt"
    )
