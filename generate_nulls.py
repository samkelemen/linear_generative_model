...
from group_model import GroupLevelModel
...
class GenerateNulls(GroupLevelModel):
    ...
    def fc_randomization(self, subject_id, subject_ids_dict):
        ...
        for null_num in range(self.num_nulls):
            ...
            randomized_data, _ = bct.null_model_und_sign(
                subject.fc_matrix, bin_swaps=10, wei_freq=1
            )
    ...
    def calculate_gl_null_rules(self):
        ...
        for null_num in range(self.num_nulls):
            ...
            for subject in self.subjects:
                fc_null = self.fc_null_symmetric_mod(
                    self.get_fc_null(subject.subject_id, null_num)
                )
            ...
            null_rules = self.inverse_symmetric_modification(
                flat_null_rules, mat_size=np.shape(self.subjects[1].sc_matrix)
            )
...
def main(pre_resection):
    ...
    null_generator = GenerateNulls(
        subject_ids_dict.keys(), 100, path, tumor_ds=True, pre_resection=True
    )
